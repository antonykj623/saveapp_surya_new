...
class _SlidebleListState1 extends State<Tasks> with SingleTickerProviderStateMixin {
  ...
  Future<void> _scheduleNotification() async {
    ...
    switch (dropdownvalu) {
      case "OneTime":
        await notificationService.scheduleOnetimeNotification(
          ...
          body: "This is your one-time notification.", time: tzDateTime,
      ...
      case "Daily":
        await notificationService.scheduleDailyNotification(
          ...
          body: "This repeats every day at the selected time.",
          time: tzDateTime,
        );
        break;
      ...
      case "Weekly":
        await notificationService.scheduleWeeklyNotification(
          ...
          body: "This repeats every week at the same day & time.",
          time: tzDateTime,
        );
        break;
      ...
      case "Monthly":
        await notificationService.scheduleMonthlyNotification(
          ...
          body: "This repeats every month on this date & time.",
          time: tzDateTime,
        );
        break;
      ...
      case "Yearly":
        await notificationService.scheduleYearlyNotification(
          id: 5,
          title: "Yearly Notification",
          body: "This repeats every year on this date & time.",
          time: tzDateTime,
        );
        break;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('$repeatOption notification scheduled!')),
    );
  }
  ...
  Future<void> _pickDate(BuildContext context) async {
    ...
    if (pickedDate != null) {
      setState(() {
       ...
       enddateCtl1.text = "${pickedDate.day}/${pickedDate.month.toString().padLeft(2, '0')}/${pickedDate.year.toString().padLeft(2, '0')}";
       print(enddateCtl1.text);
       // reminddateCtl1.text = DateFormat('yyyy-MM-dd').format(pickedDate);
      });
    }
  }
  ...
  Future<void> saveTaskToDB() async {
    ...
    try {
      ...
      //  final DateTime remindDateUpTo = DateFormat('dd/MM/yyyy').parse(reminddateCtl1.text.trim());
      // DateTime? remindDateUpTo = reminddateCtl1.text.isNotEmpty
      //     ? DateFormat('dd/MM/yyyy').parse(reminddateCtl1.text.trim())
      //     : null;


      DateTime? remindDateUpTo;
      if (reminddateCtl1.text.trim().isNotEmpty) {
        remindDateUpTo = DateFormat('dd/MM/yyyy').parse(reminddateCtl1.text.trim());
      }
      Future<void> saveRow(DateTime date, DateTime remindDate) async {
        Map<String, dynamic> taskData = {
          ...
          //  remindDateUpTo != null ? DateFormat('dd/MM/yyyy').format(remindDateUpTo) : "no data",
      ...
      if(dropdownvalu == "OneTime") {
       ...
       await saveRow(loopDate,loopEndDate);
        await ns.scheduleDailyNotification(id: 1,
            title:  "OneTime Notification", body: "This repeats every day at the selected time.",
            time: selectedDateTime!);
      //   await notificationService.scheduleNotification(
      //     id: 1,
      //     title: "Scheduled Notification",
      //     body: "This is your one-time notification.",
      // scheduledDate: selectedDateTime!,
      //  //   scheduledDate: DateTime.parse(reminddateCtl1.text),
      //
      //   );
      }
      else {
        DateTime loopDate = startDate;
        DateTime loopEndDate = endDate;
        while (loopDate.isBefore(remindDateUpTo!) || loopDate.isAtSameMomentAs(remindDateUpTo)) {
          // ✅ Only save if loopEndDate is not beyond remindDateUpTo
          if (loopEndDate.isAfter(remindDateUpTo)) {
            break;
          }
          ...
          switch (dropdownvalu) {
            case "Daily":
              ...
              await ns.scheduleDailyNotification(id: 2,
                  title: "Daily Notification", body: "This repeats every day at the selected time.",
                  time: tzDateTime);
              // await notificationService.scheduleDailyNotification(
              //   id: 2,
              //   title: "Daily Notification",
            ...
            case "Quarterly":
              ...
              await notificationService.scheduleYearlyNotification(
                id: 5,
                title: "Quarterly Notification",
                body: "This repeats every 3 months on this date & time.",
                time: tzDateTime,
              );
              break;
            case "Half Yearly":
              ...
              await notificationService.scheduleYearlyNotification(
                id: 6,
                title: "Half yearly  Notification",
                body: "This repeats every 6 months on this date & time.",
                time: tzDateTime,
              );
...
//                             final DateTime startDate = DateFormat('dd/MM/yyyy').parse(startdateCtl.text.trim());
//                             DateTime endDate = DateFormat('dd/MM/yyyy').parse(enddateCtl1.text.trim());
...
//
//                             } else {
//                               // ✅ Repeat until remind date is reached
//                               while(remindupto != null && (loopDate.isBefore(remindupto) ||
...
//
//                                   default:
//                                   // in case dropdownvalu is not matched
//                                     break;
//                                 }
//                               }
//                             }
