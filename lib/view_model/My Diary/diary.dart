...
class _DiaryState extends State<Diary> with TickerProviderStateMixin {
  static const String _selectedDateKey = 'diary_selected_date';
  static const String _selectedSubjectKey = 'diary_selected_subject';
...
//_loadSubjectsByDate(selectedDate);
  ...
  Future<void> _refreshData() async {
    ...
    await _loadSubjectsByDate(selectedDate);
    await _loadData(subject: selectedSubject, date: selectedDate);
    await _saveDiaryFilters();
  ...
  Future<void> _saveDiaryFilters() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_selectedDateKey, selectedDate.toIso8601String());
    await prefs.setString(_selectedSubjectKey, selectedSubject ?? '');
  }

  Future<void> _restoreDiaryFilters() async {
    final prefs = await SharedPreferences.getInstance();
    final savedDate = prefs.getString(_selectedDateKey);
    final savedSubject = prefs.getString(_selectedSubjectKey);

    selectedDate =
    savedDate != null && savedDate.isNotEmpty
        ? (DateTime.tryParse(savedDate) ?? DateTime.now())
        : DateTime.now();

    selectedSubject =
    savedSubject != null && savedSubject.isNotEmpty
        ? savedSubject
        : null;

    _hasUserSelectedFilter = true; // ✅ IMPORTANT
  ...
  Future<void> _loadSubjectsByDate(DateTime selectedDate) async {
    ...
    rawData
      ...
      try {
        ...
        final sub = decoded['subject']?.toString().trim();
    ...
    setState(() {
      ...
      if (currentSubject != null && subjectList.contains(currentSubject)) {
        selectedSubject = currentSubject;
      } else {
        if (currentSubject != null && subjectList.contains(currentSubject)) {
          selectedSubject = currentSubject;
        } else {
          selectedSubject = null; // ✅ don't auto-pick
        }
        // selectedSubject = subjectList.isNotEmpty ? subjectList.first : null;
      }

      if (!_hasUserSelectedFilter) {
        datalist = [];
      }
    });
  }
  ...
  Future<void> exportPDF() async {
    ...
    final file = File(
      "${dir!.path}/diary_${DateTime.now().millisecondsSinceEpoch}.pdf",
    );

    await file.writeAsBytes(await pdf.save());

    await Share.shareXFiles([XFile(file.path)], text: "My Diary Export");
  }
                ...
                Row(
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      onPressed: () async {
                        await DatabaseHelper().deleteByKeyId(
                          'DIARY_table',
                          keyid,
                        );
      ...
      body: Container(
        decoration: const BoxDecoration(
          gradient: LinearGradient(
            colors: [Color(0xFF112eea), Color(0xFF121ba2), Color(0xFFF111fb)],
          ),
        ),
        child: Column(
          children: [
            ...
            Padding(
              ...
              GemCalendar(
                ...
                onDateSelected: (date) async {
                  ...
                  await _loadSubjectsByDate(date);
                  await _saveDiaryFilters();
                  await _loadData(
                    subject: selectedSubject,
                    date: selectedDate,
                ...
                children: [
                  ...
                  AnimatedCrossFade(
                    ...
                    firstChild: SizedBox(
                      height: 320,
                      child:
                      !_hasUserSelectedFilter
                          ? const Center(
                        child: Text(
                          "Select date or subject to view entries",
...
//
//     selectedSubject =
//         savedSubject != null && savedSubject.isNotEmpty ? savedSubject : null;
//
//     await _loadSubjectsByDate(selectedDate);
//     await _saveDiaryFilters();
//   }
